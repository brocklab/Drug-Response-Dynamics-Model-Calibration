...
%a linear time delay on the cell death term.

function [forward_model] = PM_Forward_v16(N_init,t_vector,g_0,N_max,f_r,g_r,k_D,t_d,t_r)
...
for i = 2:lgth
    if t_vector(i)<t_d
        k = k_D-(k_D+g_0)*(1-t_vector(i)/t_d);
    else
        k = k_D;
    end
