...
%a constant cell death term (model 3).

function [forward_model] = Model_3_RSForward(N_init,t_vector,N_max,f_r,g_r,k_D,t_r)

%Make a matrix to hold cell number values
lgth = length(t_vector);
cell_number = zeros(lgth,3);

%identify initial time, so that we can correctly adjust for the
%proliferation delay.
t_init = t_vector(1,1);
